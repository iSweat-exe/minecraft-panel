...
export const useServerStatsStore = create<StatsState>()(
    persist(
        (set) => ({
            rawPoints: [],
            hourPoints: [],
        ...
        {
            name: 'minecraft-panel-stats',
            partialize: (state) => ({ 
                rawPoints: state.rawPoints, 
                hourPoints: state.hourPoints, 
                dayPoints: state.dayPoints,
                lastHourUpdate: state.lastHourUpdate,
                lastDayUpdate: state.lastDayUpdate
            }),
        }
    )
);
