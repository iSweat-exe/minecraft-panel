...
export const ModCard: React.FC<ModCardProps> = ({ mod, onInstall, isInstalling }) => {
    ...
    return (
        <div className="bg-background border border-border rounded-lg p-4 flex flex-col gap-3 hover:border-primary/50 transition-colors">
            <div className="flex gap-4">
                <img 
                    src={mod.icon_url || 'https://docs.modrinth.com/img/logo.svg'} 
                    alt={mod.title} 
                    className="w-16 h-16 rounded-md object-cover bg-surface"
                />
                <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-foreground text-lg truncate" title={mod.title}>{mod.title}</h3>
                    <div className="text-sm text-muted-foreground truncate">par {mod.author}</div>
                    
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                        <span className="flex items-center gap-1 text-xs text-muted-foreground bg-surface px-2 py-0.5 rounded">
                        ...
                        {mod.display_categories.filter(c => ['fabric', 'forge', 'quilt', 'neoforge'].includes(c)).map(loader => (
                            <span key={loader} className="text-xs px-2 py-0.5 rounded capitalize bg-primary/10 text-primary">
                                {loader}
                            </span>
                        ))}
                    </div>
