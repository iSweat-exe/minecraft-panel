...
pub async fn perform_ping(host: &str, state: &tauri::State<'_, SshState>) -> Result<McPing, AppError> {
    ...
    let result = tokio::task::spawn_blocking(move || -> Option<McPing> {
        ...
        // Try port 80 first
        if let Ok(mut addrs) = format!("{}:80", host_clone).to_socket_addrs() {
        ...
        if stream_opt.is_none() {
            if let Ok(mut addrs) = format!("{}:25565", host_clone).to_socket_addrs() {
                if let Some(addr) = addrs.next() {
                    if let Ok(s) = TcpStream::connect_timeout(&addr, Duration::from_secs(2)) {
                        stream_opt = Some(s);
                        used_port = 25565;
        ...
        let host_bytes = host_clone.as_bytes();
        ...
        if stream.write_all(&status_req).is_err() {
            return None;
        }
