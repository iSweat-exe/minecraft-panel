...
impl DockerManager {
    ...
    pub async fn reconstruct_spec(&self, server_id: &str) -> Result<protocol::docker::ContainerSpec> {
        ...
        if let Some(port_bindings) = inspect.host_config.as_ref().and_then(|hc| hc.port_bindings.as_ref()) {
            for (port_str, bindings) in port_bindings {
                let Some(bindings) = bindings else { continue };
                for binding in bindings {
                    let Some(host_port_str) = &binding.host_port else { continue };
        ...
        if let Some(binds) = inspect.host_config.as_ref().and_then(|hc| hc.binds.as_ref()) {
            for bind in binds {
        ...
        if let Some(host_config) = &inspect.host_config {
            resources.memory_limit_bytes = host_config.memory.and_then(|m| if m > 0 { Some(m) } else { None });
