...
async fn handle_pty_socket(mut socket: WebSocket) {
    ...
    // Important: drop slave in parent to avoid EOF issues
    drop(pair.slave);

    let mut reader = match pair.master.try_clone_reader() {
        Ok(r) => r,
        Err(e) => {
            let _ = socket.send(Message::Text(format!("Failed to clone PTY reader: {}", e).into())).await;
            return;
        }
    };
    let mut writer = match pair.master.take_writer() {
        Ok(w) => w,
        Err(e) => {
            let _ = socket.send(Message::Text(format!("Failed to take PTY writer: {}", e).into())).await;
            return;
        }
    };
