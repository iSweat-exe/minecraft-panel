...
use axum::Json;
use protocol::{ApiResponse, PatchServerRequest, docker::ContainerSpec};
...
pub async fn patch_server(
    ...
    let (spec_json, spec_version): (String, i64) = sqlx::query_as(
        "SELECT spec_json, spec_version FROM servers WHERE id = ?"
    )
    .bind(&server_id)
    ...
    let mut spec: ContainerSpec = serde_json::from_str(&spec_json)
        .context("Failed to deserialize ContainerSpec")?;

    // 2. Appliquer les modifications partielles
    if let Some(name) = payload.name { spec.name = name; }
    if let Some(image) = payload.image { spec.image = image; }
    if let Some(env) = payload.env { spec.env = env; }
    if let Some(ports) = payload.ports { spec.ports = ports; }
    if let Some(volumes) = payload.volumes { spec.volumes = volumes; }
    if let Some(resources) = payload.resources { spec.resources = resources; }
    if let Some(owner) = payload.owner { spec.owner = Some(owner); }
    ...
    match state
        ...
        .await
        .context(format!("Failed to remove old server container {}", server_id))
    {
        Ok(_) => {}
        Err(e) => {
