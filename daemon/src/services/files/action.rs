...
pub async fn perform_action(
    ...
    match action {
        FileAction::Rename { new_name } => {
            let new_path = crate::services::files::sanitize_path(server_id, data_dir, &new_name)?;
        ...
        FileAction::Copy { destination } => {
            let dest_path =
                crate::services::files::sanitize_path(server_id, data_dir, &destination)?;
        ...
        FileAction::Archive {
            ...
            let archive_path =
                crate::services::files::sanitize_path(server_id, data_dir, &archive_name)?;
        ...
        FileAction::Extract => {
            ...
            crate::services::files::fix_permissions_for(parent, parent).ok();
        }
    }
    Ok(())
}
