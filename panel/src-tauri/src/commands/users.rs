...
#[tauri::command]
pub async fn get_panel_users(node_url: String, node_token: String) -> Result<Vec<PanelUser>, AppError> {
    let client = DaemonClient::new(node_url.clone(), node_token.clone());
    
    // Ensure dir exists
    let _ = client.file_action("/minecraft/.panel_users", protocol::FileAction::Mkdir).await;
...
#[tauri::command]
pub async fn save_panel_user(node_url: String, node_token: String, mut user: PanelUser) -> Result<Vec<PanelUser>, AppError> {
    let client = DaemonClient::new(node_url.clone(), node_token.clone());
    ...
    if user.created_at.is_none() {
        user.created_at = Some(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs());
    }
    
    if user.uuid.is_none() {
        user.uuid = existing_idx.and_then(|idx| users[idx].uuid.clone()).or_else(|| Some(uuid::Uuid::new_v4().to_string()));
    }
...
#[tauri::command]
pub async fn delete_panel_user(node_url: String, node_token: String, username: String) -> Result<Vec<PanelUser>, AppError> {
    let client = DaemonClient::new(node_url.clone(), node_token.clone());
    let mut users = get_panel_users(node_url.clone(), node_token.clone()).await?;
...
#[tauri::command]
pub async fn verify_panel_user(node_url: String, node_token: String, username: String, password: String) -> Result<PanelUser, AppError> {
